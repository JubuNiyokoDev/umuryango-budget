...
const CURRENT_VERSION = 'v18'; // Version actuelle de l'app
const GITHUB_API = 'https://api.github.com/repos/JubuNiyokoDev/umuryango-budget/releases/latest';

interface UpdateManagerProps {
  onUpdateComplete?: () => void;
}

export default function UpdateManager({ onUpdateComplete }: UpdateManagerProps) {
  const [updateAvailable, setUpdateAvailable] = useState(false);
  const [showUpdateModal, setShowUpdateModal] = useState(false);
  const [downloadProgress, setDownloadProgress] = useState(0);
  const [updateStatus, setUpdateStatus] = useState<'checking' | 'downloading' | 'installing' | 'idle'>('idle');
        ...
        <LinearGradient colors={['#1976D2', '#42A5F5']} style={styles.modal}>
          ...
          <View style={styles.content}>
            ...
            {updateStatus === 'downloading' && (
              <View style={styles.progressContainer}>
                ...
                <Text style={styles.archText}>Architecture: {getDeviceArchitecture()}</Text>
              </View>
            )}
            ...
            {updateStatus === 'idle' && updateAvailable && (
              <View style={styles.updateInfo}>
                <Text style={styles.updateText}>
                  Une nouvelle version est disponible avec des améliorations et corrections.
                </Text>
                <Text style={styles.sizeText}>
                  Taille optimisée pour votre appareil ({getDeviceArchitecture()})
                </Text>
              </View>
            )}
          </View>
          ...
          {updateStatus === 'idle' && updateAvailable && (
            <View style={styles.actions}>
              ...
              <TouchableOpacity style={styles.updateButton} onPress={downloadUpdate}>
...
const styles = StyleSheet.create({
  ...
  progressBar: {
    width: '100%', height: 8, marginTop: 15,
  ...
  updateText: { fontSize: 16, color: '#fff', textAlign: 'center', lineHeight: 22 },
  sizeText: { fontSize: 14, color: 'rgba(255,255,255,0.8)', textAlign: 'center', marginTop: 10 },
